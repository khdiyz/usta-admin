...
function MasterEditPage() {
  ...
  const [uploadingWorkPhotos, setUploadingWorkPhotos] = useState({});
  ...
  useEffect(() => {
    const fetchMasterData = async () => {
      ...
      try {
        const result = await makeApiCall(`${API_URL}/masters/${masterId}`, { method: 'GET' });
        ...
        setFormData({
          ...
          turonId: result.turonId || '',
          experience: {
            years: result.experience?.years || 0,
            months: result.experience?.months || 0
          },
          location: {
            countryId: result.location?.countryId || '',
            regionId: result.location?.regionId || '',
            districtId: result.location?.districtId || ''
          },
          serviceIds: result.serviceIds || [],
          workPhotos: result.workPhotos || []
        });
        
        // Set profile photo preview if exists
        if (result.profilePhoto) {
          setProfilePhotoPreview(`${FILE_URL}/${result.profilePhoto}`);
        }
        
        // Set work photos
        if (result.workPhotos && result.workPhotos.length > 0) {
          const workPhotoInputsData = result.workPhotos.map(photoId => ({
            id: photoId,
            file: null,
            previewUrl: `${FILE_URL}/${photoId}`,
            uploading: false
          }));
          setWorkPhotoInputs(workPhotoInputsData);
        }
        
        setLoadingMaster(false);
      } catch (error) {
        toast.error('Usta ma\'lumotlarini yuklashda xatolik: ' + error.message);
        setLoadingMaster(false);
        navigate('/masters');
      }
    };
  ...
  // Load regions when country changes or on initial load
  useEffect(() => {
  ...
  // Load districts when region changes or on initial load
  useEffect(() => {
  ...
  const fetchRegions = async (countryId) => {
    ...
    try {
      ...
      // Don't reset region and district when editing
      // This ensures we maintain the selected values during edit
    } catch (error) {
  ...
  const fetchDistricts = async (regionId) => {
    ...
    try {
      ...
      // Don't reset district when editing
    } catch (error) {
  ...
  // File upload function using direct fetch
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await makeApiCall(`${API_URL}/masters/${masterId}`, {
        method: 'PUT',
        body: JSON.stringify(dataToSubmit),
        headers: { 'Content-Type': 'application/json' }
      });

      toast.success('Usta ma\'lumotlari muvaffaqiyatli yangilandi');
      navigate('/masters');
    } catch (error) {
      toast.error(`Ustani yangilashda xatolik: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCancel = () => {
    showModal({
      ...
      message: 'O\'zgarishlarni saqlamasdan chiqmoqchimisiz?',
  ...
  if (loadingMaster) {
    return <div className="p-6 text-center text-gray-500">Usta ma'lumotlari yuklanmoqda...</div>;
  }

  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">Usta ma'lumotlarini tahrirlash</h1>
        <button
